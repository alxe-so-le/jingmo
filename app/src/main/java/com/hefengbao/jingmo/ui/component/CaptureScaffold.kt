...
@Composable
fun CaptureScaffold(
    viewModel: CaptureViewModel = hiltViewModel<CaptureViewModel>(),
    modifier: Modifier = Modifier,
    onBackClick: () -> Unit,
    content: @Composable (textColor: Color, textSize: Int) -> Unit
) {
        ...
        appStatus?.let {
            Column(
                modifier = Modifier
                    ...
                    .background(it.captureBackgroundColor)
                    .padding(horizontal = it.capturePaddingX.dp, vertical = it.capturePaddingY.dp),
            ) {
                content.invoke(it.captureTextColor, it.captureTextSize)
                Row(
                    modifier = Modifier
                        .align(Alignment.CenterHorizontally)
                        .padding(top = 32.dp)
                ) {
                    Text(text = "©京墨文库App", color = it.captureTextColor)
                }
            }
            BottomBar(
                context = context,
                ...
                colors = colors,
                textSize = it.captureTextSize,
                paddingX = it.capturePaddingX,
                paddingY = it.capturePaddingY,
                onTextColorChange = viewModel::setCaptureTextColor,
                onTextSizeChange = viewModel::setCaptureTextSize,
                onBackgroundColorChange = viewModel::setCaptureBackgroundColor,
                onPaddingXChanger = viewModel::setCapturePaddingX,
                onPaddingYChanger = viewModel::setCapturePaddingY,
            )
        }
    }
}
...
private fun BottomBar(
    ...
    onPaddingXChanger: (Int) -> Unit,
    onPaddingYChanger: (Int) -> Unit
) {
    ...
    if (showBottomSheet) {
        ModalBottomSheet(
            ...
            LazyColumn(
                ...
                item {
                    Text(
                        text = stringResource(R.string.capture_padding),
                ...
                item {
                    Column(
                        ...
                        Row(
                            ...
                            Text("左右边距：${paddingX}")
                            Slider(
                                value = paddingX.toFloat(),
                                onValueChange = { onPaddingXChanger(it.toInt()) },
                                steps = 5,
                                valueRange = 16f..64f
                            )
                        }
                        Row(
                            ...
                            Text("上下边距：${paddingY}")
                            Slider(
                                value = paddingY.toFloat(),
                                onValueChange = { onPaddingYChanger(it.toInt()) },
                                steps = 13,
                                valueRange = 16f..128f
                            )
                        }
                ...
                item {
                    Text(
                        text = stringResource(R.string.capture_text_size),
                        style = MaterialTheme.typography.titleMedium
                    )
                }
                item {
                    Row(
                        ...
                        OutlinedIconButton(onClick = {
                            ...
                            Icon(imageVector = Icons.Outlined.TextDecrease, contentDescription = "")
                        }
                        Text(
                        ...
                        OutlinedIconButton(onClick = {
                            ...
                            Icon(imageVector = Icons.Outlined.TextIncrease, contentDescription = "")
                        }
                    }
                }
                item {
                    Text(
                        text = stringResource(R.string.capture_text_color),
                        style = MaterialTheme.typography.titleMedium
                    )
                }
                item {
                ...
                item {
                    Text(
                        text = stringResource(R.string.capture_background_color),
                        style = MaterialTheme.typography.titleMedium
                    )
                }
                item {
